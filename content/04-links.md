...
Soft links (symbolic links) point to existing files and can span multiple filesystems. They are special files that reference another file.
...
If the original file is deleted, the soft link becomes a dangling link. If a new file is created with the same name, the link points to that new file instead.
